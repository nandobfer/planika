...
export const useTrip = (_tripId: string) => {
    ...
    const { data, refetch } = useQuery({ queryKey: ["trip", tripId], queryFn: () => fetchTrip(), initialData: null })
    ...
    return { trip, loading, acceptInvitation, refetch, tripId, authenticatedApi, user, setTrip }
}
